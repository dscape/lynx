...
//
// UDP packet's maximum size
//
var UDP_PACKET_MAX_SIZE = 1472;

// The delimeter between metrics
var DELIMETER = '\n';

function splitDataToChunks(data) {
  var limit, chunk;
  chunks = [];
  while (true) {
    if (!data.length) {
    ...
    if (data.length > UDP_PACKET_MAX_SIZE) {
      limit = data.substring(0, UDP_PACKET_MAX_SIZE).lastIndexOf(DELIMETER)
    } else {
      limit = data.length;
    }

    chunks.push(data.substring(0, limit + 1));
    data = data.substring(limit + 1, data.length);
  }
  return chunks;
}
...
Lynx.prototype.send = function send(stats, sample_rate) {
  ...
  // Send the data
  //
  this.emit('data', send_data);

  var self = this;

  send_data = splitDataToChunks(send_data);
  send_data.forEach(function (data) {
    console.log('SEND', data);
    var buffer = new Buffer(data);
    socket.send(buffer, 0, buffer.length, self.port, self.host, noop);
  });
};
